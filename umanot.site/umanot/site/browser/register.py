...
class BaseRegistrationForm(PageForm):
    ...
    def handle_join_success(self, data):
        ...
        send_mail = True
        auth = mt.getAuthenticatedMember()
        ...
        if send_mail:
            message = u'<p>Un nuovo utente si è iscritto al portale</p>'
            message += u'<p>Nome: %s</p>' % data.get('fullname', '')
            message += u'<p>E-mail: %s</p>' % data.get('email')
